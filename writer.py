...
def write_qc_report(wb, qc_df: pd.DataFrame, fail_dict: dict,
                    ...
                    ngs_path: str = "", summary_df: pd.DataFrame | None = None):
    ...
    for r_idx, (_, row) in enumerate(qc_df.iterrows(), start=2):
        ...
        row_vals = [
            bam_path if c == "bam_path"
            else _bytes_to_human(row.get(c, "")) if c == "CleanData" or c == "cleanData"
            else row.get(c, "")
            for c in headers
        ]
        ws.append(row_vals)
...
def write_germnoic_review(wb, df: pd.DataFrame, cfg: ProductConfig):
    _write_snv_sheet(wb, "GermNonIC_Review", df, cfg.somatic_review_cols, cfg)
...
def write_cnv(wb, cnv_df: pd.DataFrame):
    ...
    # 条件格式：CNV_Flag = "Yes" → 橙色（Excel 中动态生效）
    if "CNV_Flag" in col_idx:
        cl = get_column_letter(col_idx["CNV_Flag"])
        ws.conditional_formatting.add(
            f"A2:{get_column_letter(ncols)}{len(cnv_df) + 1}",
...
# ─────────────────────────────────────────────
# AmpliconStat 透视表（列=Sample，行=Amplicon，值=RoT）
# ─────────────────────────────────────────────
