...
def normalize_cds(raw: str) -> str:
    """
    从 CDSChange 字段提取核心变异描述。
    格式固定为冒号分隔：NM_xxx:exon/intron:c.xxx:p.xxx:p.xxx
    取第3段（index=2）即 c.xxx 部分。
    """
    parts = str(raw).strip().split(":")
    return parts[2].strip() if len(parts) >= 3 else str(raw).strip()
...
def process_hot_somatic(
    ...
    if "Tags" in hot_df.columns and cfg.tag_filters:
        pattern = "|".join(cfg.tag_filters)
        mask = hot_df["Tags"].astype(str).str.contains(pattern, case=False, na=False)
...
def process_somatic(
    ...
    if "Tags" in somatic_df.columns and cfg.tag_filters:
        pattern = "|".join(cfg.tag_filters)
        mask = somatic_df["Tags"].astype(str).str.contains(pattern, case=False, na=False)
...
def process_discard_rescue(
    ...
    # 条件3：Tags 含关键词 OR Gene 在指定列表
    tag_cond = pd.Series(False, index=df.index)
    if "Tags" in df.columns and cfg.rescue_tag_keywords:
        pattern = "|".join(cfg.rescue_tag_keywords)
        tag_cond = df["Tags"].astype(str).str.contains(pattern, case=False, na=False)
